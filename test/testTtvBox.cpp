...
int main(int argc, char const *argv[]) {
  ...
  tag = 1;
  {
    ...
    TTV_LOGI("getNumbericalValue() succeded, tag [%d], value [%d].", tag,
             value);
    tag++;
  }
  {
...

  {
    ...
    TTV_LOGI("getNumbericalValue() succeded, tag [%d], value [%ld]", tag,
             value);
    tag++;
  }
  {
  ...
  }
  {
    ...
    TTV_LOGI("getNumbericalValue() succeded, tag [%d], value [%lf]", tag,
             value);
    tag++;
  }

  {
    ...
    TTV_LOGI("getStringValue() succeded, tag [%d], value [%s]", tag,
             value.c_str());
    tag++;
  }
